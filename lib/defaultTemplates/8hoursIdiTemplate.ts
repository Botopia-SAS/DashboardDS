...
/**
 * 8 Hours IDI Certificate Template
 * Uses configuration from certificateConfigurations.ts
 */
export const get8HoursIdiTemplate = (): Omit<CertificateTemplate, '_id' | 'createdAt' | 'updatedAt'> => {
  const config = CERTIFICATE_CONFIGURATIONS['8-HOURS-IDI'];
