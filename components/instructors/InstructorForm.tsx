...
const InstructorForm = ({ initialData }: { initialData?: InstructorData }) => {
    ...
    const [schedule, setSchedule] = useState<{ date: string; slots: { start: string; end: string; booked: boolean; recurrence: string }[] }[]>([]);
    ...
    useEffect(() => {
        ...
        console.log("📆 Eventos de FullCalendar actualizados:", JSON.stringify(events, null, 2));
    }, [schedule]);
    ...
    const [copiedSlot, setCopiedSlot] = useState<{ duration: number; booked: boolean; recurrence: string } | null>(null);
    ...
    const handleDateSelect = (selectInfo: any) => {
        ...
        console.log("📌 handleDateSelect ejecutado con datos:", { start, end, date });
    ...
    const handleUpdateSlot = () => {
        ...
        console.log("📌 CurrentSlot recibido en update:", JSON.stringify(currentSlot, null, 2));
        console.log("📌 Estado ANTES de actualizar `schedule`:", JSON.stringify(schedule, null, 2));
        ...
        setSchedule((prevSchedule) => {
            ...
            console.log("✅ Estado DESPUÉS de actualizar `schedule`:", JSON.stringify(newSchedule, null, 2));
    
            return newSchedule;
        });
    
        setTimeout(() => {
            console.log("📆 Eventos de FullCalendar después de actualización:", JSON.stringify(schedule, null, 2));
        }, 200);
    ...
    const handleSaveSlot = () => {
        ...
        setSchedule((prevSchedule) => {
            ...
            if (existingDayIndex !== -1) {
                ...
                updatedSchedule = [...prevSchedule, { date: slotDate, slots: [currentSlot] }];
            }
            ...
            if (currentSlot.recurrence === "Weekly") {
                ...
                for (let i = 1; i <= 4; i++) { // Generamos para las próximas 4 semanas
                    ...
                    const existingRecurringDayIndex = updatedSchedule.findIndex((day) => day.date === newDateString);
    ...
    const onSubmit = async (values: InstructorData) => {
        ...
        try {
            ...
            const res = await fetch(`/api/instructors`, {
                ...
                body: JSON.stringify({
                    instructorId: (initialData as any)?._id,
                    ...values,
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                    ...
                    <Dialog open={isModalOpen} onClose={() => setIsModalOpen(false)} className="fixed inset-0 flex items-center justify-center z-50">
                        <div className="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full">
                            ...
                            <Input
                                type="time"
                                value={currentSlot?.start ? currentSlot.start.split("T")[1].slice(0, 5) : ""}
                                onChange={(e) =>
                                    setCurrentSlot((prev) =>
                                        prev ? { ...prev, start: `${prev.start.split("T")[0]}T${e.target.value}` } : prev
                                    )
                                }
                            />
                            ...
                            <Input
                                type="time"
                                value={currentSlot?.end ? currentSlot.end.split("T")[1].slice(0, 5) : ""}
                                onChange={(e) =>
                                    setCurrentSlot((prev) =>
                                        prev ? { ...prev, end: `${prev.end.split("T")[0]}T${e.target.value}` } : prev
                                    )
                                }
                            />
                            ...
                            <Select
                                ...
                                onChange={(e) =>
                                    setCurrentSlot((prev) => prev ? { ...prev, recurrence: e.target.value } : prev)
                                }
                            >
                            ...
                            <div className="mt-4 flex justify-between">
                                ...
                                <Button onClick={() => {
                                    if (currentSlot) {
                                        ...
                                        const startMinutes = new Date(currentSlot.start).getMinutes();
                                        ...
                                        setCopiedSlot({ booked: currentSlot.booked, recurrence: currentSlot.recurrence, duration });
                                ...
                                <Button onClick={() => {
                                    if (copiedSlot && currentSlot) {
                                        ...
                                        newEndTime.setMinutes(startTime.getMinutes() + copiedSlot.duration);

                                        const formattedStart = `${currentSlot.start.split("T")[0]}T${startHour.toString().padStart(2, "0")}:${startMinutes.toString().padStart(2, "0")}`;
                                        const formattedEnd = `${currentSlot.start.split("T")[0]}T${newEndTime.getHours().toString().padStart(2, "0")}:${newEndTime.getMinutes().toString().padStart(2, "0")}`;
                                        ...
                                        setSchedule((prevSchedule) => {
                                            const updatedSchedule = prevSchedule.map(day => ({
                                                ...
                                                slots: day.slots.filter(slot => {
                                                    const normalizedSlotStart = slot.start.split(":").slice(0, 2).join(":");
                                                    const normalizedSlotEnd = slot.end.split(":").slice(0, 2).join(":");
                                                    const normalizedCurrentStart = currentSlot.start.split(":").slice(0, 2).join(":");
                                                    const normalizedCurrentEnd = currentSlot.end.split(":").slice(0, 2).join(":");

                                                    console.log("Comparando:");
                                                    console.log("🔹 Slot Start:", normalizedSlotStart, "vs", normalizedCurrentStart);
                                                    console.log("🔹 Slot End:", normalizedSlotEnd, "vs", normalizedCurrentEnd);

                                                    return !(normalizedSlotStart === normalizedCurrentStart && normalizedSlotEnd === normalizedCurrentEnd);
                                                })
